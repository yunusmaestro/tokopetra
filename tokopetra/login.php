...
<body>
	...
	<div class="text-center">
		<button type="button" class="btn btn-outline-primary btn-lg bg-primary text-white" data-toggle="modal" data-target="#myModal">Sign In</button>
		<button type="button" class="btn btn-outline-success btn-lg bg-success text-white" data-toggle="modal" data-target="#myModal2">Sign Up</button>
	</div>
					...
					<div class="modal-body" method="POST">
						<form class="form-signin">
							...
							<p class="text-center">Belum punya akun Tokopetra? 
								<a href="signup.html" class="text-*-right" data-dismiss="modal" data-toggle="modal" data-target="#myModal2">Daftar</a>
							</p>
						</form>
					</div>
				</div>
			...
			<div class="modal-dialog vertical-align-center">
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<div class="form-label-group">
							<input type="number" id="inputPostalCode" class="form-control" placeholder="Postal Code" required>
						...
						<button type="submit" id="signup" class="btn btn-lg btn-success btn-block text-uppercase">Sign up</button>
						<hr class="my-2">
					</div>
				</div>
			</div>
	...
	<script>
			...
			$.ajax({
				...
				success: function(result) {
					...
					result.forEach(function(kota){
						var row = $("<option value=" + kota['id'] + ">" + kota['nama'] +"</option>");
		...
		$("[id='signup']").click(function(){
			...
			$.ajax({
				url: "signup.php",
				method: "POST",
				data : {name:varname, date:vardate, phone:varphone, city:varcity, postcode:varpostcode, address:varaddress, account:varaccount, email:varemail, password:varpassword},
				success: function(result) {
        			if(result.notif){
        				alert(result.notif);
        			}
